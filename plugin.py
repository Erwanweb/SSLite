...
class BasePlugin:
    ...
    def verifAlarmState(self):
        ...
        DayoflastactiononAlarmLeval = LastActionAlarmLevel.today().strftime("%u")
        Today = datetime.now().strftime("%u")
        LastActionDayintext = Today
        if Today == DayoflastactiononAlarmLeva
        ...
        Domoticz.Debug("----> french : {} at {} ".format(Todayintext, Time))
        ...
        if Devices[1].nValue == 0:
            ...
            if Devices[2].nValue == 1:
                if self.AlarmLevel == 1:
                    ...
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode périmétrique depuis {}".format(Time))
                if self.AlarmLevel == 2:
                    Domoticz.Log("--Alarm - Check State : Active level Night...")
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode nuit depuis {}".format(Time))
                if self.AlarmLevel == 3:
                    Domoticz.Log("--Alarm - Check State : Active level Total...")
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode général depuis {}".format(Time))
            else:
                if self.AlarmLevel == 1:
