        ...
        if Devices[1].nValue == 0:
            ...
            if Devices[2].nValue == 1:
                if self.AlarmLevel == 1:
                    ...
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode périmétrique")
                if self.AlarmLevel == 2:
                    Domoticz.Log("--Alarm - Check State : Active level Night...")
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode nuit")
                if self.AlarmLevel == 3:
                    Domoticz.Log("--Alarm - Check State : Active level Total...")
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode général")
            else:
                if self.AlarmLevel == 1:
                    Domoticz.Log("--Alarm - Check State : Initialization level Perimetral...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode périmétrique mais elle est est en cours dinitialisation")
                if self.AlarmLevel == 2:
                    Domoticz.Log("--Alarm - Check State : Initialization level Night...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode nuit mais elle est est en cours dinitialisation")
                if self.AlarmLevel == 3:
                    Domoticz.Log("--Alarm - Check State : Initialization level Total...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode général mais elle est est en cours dinitialisation")
