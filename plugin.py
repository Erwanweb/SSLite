            ...
            if Devices[2].nValue == 1:
                if Devices[3].nValue == 1:
                    if Devices[5].nValue == 1:
                        ...
                        Devices[8].Update(nValue = 3,sValue = "INTRUSION DETECTED - PERIMETRAL")
                        self.Intrusion = True
                        self.Detectionchangedtime = datetime.now()
        ...
        if self.NightDetection:
            if Devices[5].nValue == 1:
                ...
                if self.Detectiondelay == 0:
                    ...
                    Devices[8].Update(nValue = 3,sValue = "INTRUSION DETECTED - GROUPE 1")
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
                else :
        ...
        if self.NormalDetection:
            if Devices[5].nValue == 1:
                ...
                if self.Detectiondelay == 0:
                    Domoticz.Log("New intrusion...")
                    Devices[5].Update(nValue = 1,sValue = Devices[5].sValue)
                    Devices[8].Update(nValue = 3,sValue = "INTRUSION DETECTED - GROUPE 2")
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
                else :
                    if self.Detectiontempo + timedelta(seconds = self.Detectiondelay) <= now:
            ...
            if not self.Alarm:
                if self.Alarmondelay == 0:
                    ...
                    if self.Detectionchangedtime + timedelta(seconds = self.Alarmondelay) <= now:
                        ...
                        self.Alarm = True
                        Devices[7].Update(nValue = 1,sValue = Devices[7].sValue)
                        Devices[8].Update(nValue = 4,sValue = "INTRUSION DETECTED - NO IDENTIFICATION DURING ALLOWED TIME - ALARM")
                    else:
