...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 7 not in Devices:
            Domoticz.Device(Name="Alarm Log", Unit=7, TypeName="Text", Used=1).Create()
        ...
        if (Unit == 1):
            ...
            if self.AlarmReady :
                if (Devices[1].sValue == "20"):
                    ...
                    Devices[7].Update(nValue = 1,sValue = "Perimetral protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()
                elif (Devices[1].sValue == "30"):
                    Devices[1].Update(nValue = 1,sValue = "30")
                    Devices[7].Update(nValue = 1,sValue = "Night protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()
                elif (Devices[1].sValue == "40"):
                    Devices[1].Update(nValue = 1,sValue = "40")
                    Devices[1].Update(nValue = 1,sValue = "Total protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()
            else :
