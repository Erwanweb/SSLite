...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 7 not in Devices:
            Domoticz.Device(Name="Alarm Log", Unit=7, TypeName="Alert", Used=1).Create()
            devicecreated.append(deviceparam(7, 0, ""))  # default is clear
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if (Unit == 1):
            ...
            if (Devices[1].sValue == "10"):
                ...
                Devices[7].Update(nValue = 1,sValue = "Alarm Disarmed")
                self.NightNewDetection = False
                self.NightDetection = False
                self.NormalNewDetection = False
                self.NormalDetection = False
            ...
            if self.AlarmReady :
                if (Devices[1].sValue == "20"):
                    ...
                    Devices[7].Update(nValue = 2,sValue = "Perimetral protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()

                elif (Devices[1].sValue == "30"):
                    Devices[1].Update(nValue = 1,sValue = "30")
                    Devices[7].Update(nValue = 2,sValue = "Night protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()

                elif (Devices[1].sValue == "40"):
                    Devices[1].Update(nValue = 1,sValue = "40")
                    Devices[7].Update(nValue = 2,sValue = "Total protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()
        ...
        if (Devices[1].nValue == 0):  # Surveillance is off but we check if perimetral is ok, if not, unable to turn on protection
            if Devices[3].nValue == 0:
                ...
                if (Devices[1].sValue == "0"):
                    ...
                    Devices[7].Update(nValue = 1,sValue = "Disarmed - Ready to Arm")
            else :
                self.AlarmReady = False
                if (Devices[1].sValue == "10"):
                    Devices[1].Update(nValue = 0,sValue = "0")
                    Devices[7].Update(nValue = 0,sValue = "Disarmed - Not ready - Please check perimetral sensors")
        ...
        if (Devices[1].nValue == 0):  # Surveillance is off
            ...
            if self.Armingtempo + timedelta(seconds = self.Armingondelay) <= now:
                if Devices[2].nValue == 0:
                    ...
                    Devices[7].Update(nValue = 2,sValue = "Protection Armed")
            else :
                ...
                if idx in self.DTPerimetral:  # this is one of our DT
                    if "Status" in device:
                        ...
                        if device["Status"] == "On":
                            self.Perimetraltempo = datetime.now()
                            if Devices[2].nValue == 1:
                                Devices[7].Update(nValue = 3,sValue = "Perimetral DT switch '{}' Detection".format(device["Name"]))
                    else:
            ...
            if Devices[2].nValue == 1:
                if Devices[3].nValue == 1:
                    if Devices[5].nValue == 1:
                        ...
                        Devices[7].Update(nValue = 3,sValue = "New Perimetral Intrusion Detected")
                        self.Intrusion = True
                        self.Detectionchangedtime = datetime.now()
                ...
                if idx in self.DTNightAlarm:  # this is one of our presence/motion sensors
                    if "Status" in device:
                        ...
                        if device["Status"] == "On":
                            self.NightAlarmtempo = datetime.now()
                            if Devices[2].nValue == 1:
                                Devices[7].Update(nValue = 3,sValue = "Night Zone DT '{}' Detection".format(device["Name"]))
        ...
        if self.NightDetection:
            if Devices[5].nValue == 1:
                ...
                if self.Detectiondelay == 0:
                    Domoticz.Log("New intrusion...")
                    Devices[5].Update(nValue = 1,sValue = Devices[5].sValue)
                    Devices[7].Update(nValue = 3,sValue = "New Motion Detected in night zone")
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
                else :
    ...
    def NormalMotionDetection(self):
                ...
                if idx in self.DTNormalAlarm:  # this is one of our presence/motion sensors
                    if "Status" in device:
                        ...
                        if device["Status"] == "On":
                            self.NormalAlarmtempo = datetime.now()
                            if Devices[2].nValue == 1:
                                Devices[7].Update(nValue = 3,sValue = "Normal Zone DT '{}' Detection".format(device["Name"]))
        ...
        if self.NormalDetection:
            if Devices[5].nValue == 1:
                ...
                if self.Detectiondelay == 0:
                    Domoticz.Log("New intrusion...")
                    Devices[7].Update(nValue = 3,sValue = "New Motion Detected in normal zone")
                    Devices[5].Update(nValue = 1,sValue = Devices[5].sValue)
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
                else :
    ...
    def AlarmDetection(self):
        ...
        if self.Intrusion:
            if not self.Alarm:
                if self.Alarmondelay == 0:
                    ...
                    Devices[7].Update(nValue = 4,sValue = "Intrusion Detected - Alarm")
                else :
        ...
        if self.Alarm:
            if self.Detectionchangedtime + timedelta(seconds = (self.Alarmondelay + self.Alarmoffdelay)) <= now:
                ...
                self.Alarm = False
                Devices[6].Update(nValue = 0,sValue = Devices[6].sValue)
                Devices[7].Update(nValue = 3,sValue = "Alarm reset - Protection Armed")
                self.Intrusion = False
                Devices[5].Update(nValue = 0,sValue = Devices[5].sValue)
