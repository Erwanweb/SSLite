        ...
        if (Unit == 1):
            ...
            if self.AlarmReady :
                ...
                if (Devices[1].sValue != "0"):
                    ...
                    if self.Alexa:
                        AlexaAPI("Commande de la protéction impossible. Vérifiez les capteurs périmétriques")
        ...
        if (Devices[9].nValue == 1):
            ...
            if (Devices[9].sValue == "10"):
                if self.PSControltime + timedelta(seconds = 10) <= now:
                    Domoticz.Log("Verification of PS...")
                    PerimetralDT = {}
                    ...
                    if devicesAPI:
                        for device in devicesAPI["result"]:  # parse the perimetral sensors (PS) devices
                            idx = int(device["idx"])
                            if idx in self.DTPerimetral:  # this is one of our PS
                                if "Status" in device:
                                    ...
                                    Devices[8].Update(nValue=1, sValue="Verif PAS : --- '{}' - '{}'".format(device["Name"], device["Status"]))
                                    if device["Status"] == "On":
                                        self.DTtempoPS = datetime.now()
                                        if self.Telegram:
                                           TelegramAPI("--- Actif : '{}' - '{}'".format(device["Name"], device["Status"]))
                                        if self.Alexa:
                                           AlexaAPI("Actif {}".format(device["Name"]))

                if self.ControlSensortempo + timedelta(seconds=60) <= now: #At mini 60 seconds was past since asked verif PS or MS
                    Domoticz.Debug("Verif PS done...")
                    if self.DTtempoPS + timedelta(seconds=90) >= now: #there is at mini 1 PS that was ON in the past 90 seconds
                        Domoticz.Debug("Verif PS -> SOME AR ON")
                        if self.Alexa:
                            AlexaAPI("Vérification des capteurs périmétriques terminé. Vérifiéz les capteurs actifs. Dites Alexa vérification PS")
                    else:
                        Domoticz.Debug("Verif PS -> ALL OK")
                        if self.Alexa:
                            AlexaAPI("Vérification des capteurs périmétriques terminé. Toute est OK")
                    Devices[9].Update(nValue=0, sValue="0")
                                    
            elif (Devices[1].sValue == "20"):
                if self.PSControltime + timedelta(seconds = 15) <= now:
                    Devices[9].Update(nValue=0, sValue="0")
                    Domoticz.Log("Verification of MS...")
