...
class BasePlugin:
    ...
    def __init__(self):
        ...
        self.Detectionchangedtime1 = datetime.now()
        self.Detectionchangedtime2 = datetime.now()
        self.Detectionchangedtime3 = datetime.now()
        self.loglevel = None
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if (Unit == 1):
            ...
            if (Devices[1].sValue == "10"):
                ...
                Devices[8].Update(nValue = 1,sValue = "Alarm Disarmed")
                self.NightNewDetection = False
                self.NightDetection = False
                self.NormalNewDetection = False
                self.NormalDetection = False
            ...
            if self.AlarmReady :
                if (Devices[1].sValue == "20"):
                    ...
                    Devices[8].Update(nValue = 2,sValue = "Perimetral protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()

                elif (Devices[1].sValue == "30"):
                    Devices[1].Update(nValue = 1,sValue = "30")
                    Devices[8].Update(nValue = 2,sValue = "Night protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()

                elif (Devices[1].sValue == "40"):
                    Devices[1].Update(nValue = 1,sValue = "40")
                    Devices[8].Update(nValue = 2,sValue = "Total protection Armed - Timer on period")
                    self.Armingtempo = datetime.now()
        ...
        if (Devices[1].nValue == 0):  # Surveillance is off but we check if perimetral is ok, if not, unable to turn on protection
            if Devices[3].nValue == 0:
                ...
                if (Devices[1].sValue == "0"):
                    ...
                    Devices[8].Update(nValue = 1,sValue = "Disarmed - Ready to Arm")
            else :
                self.AlarmReady = False
                if (Devices[1].sValue == "10"):
                    Devices[1].Update(nValue = 0,sValue = "0")
                    Devices[8].Update(nValue = 0,sValue = "Disarmed - Not ready - Please check perimetral sensors")
        ...
        if (Devices[1].nValue == 0):  # Surveillance is off
            ...
            if self.Alarm:  # Surveillance setting was just changed so we kill the surveillance of the zone and stop the eventual alarm
                ...
                if Devices[7].nValue == 1:
                    Devices[7].Update(nValue = 0,sValue = Devices[4].sValue)
            ...
            if self.Armingtempo + timedelta(seconds = self.Armingondelay) <= now:
                if Devices[2].nValue == 0:
                    ...
                    Devices[8].Update(nValue = 2,sValue = "Protection Armed")
                    Devices[8].Update(nValue = 2,sValue = "Surveillance started")
            else :
            ...
            if Devices[2].nValue == 1:
                self.AlarmDetection()
                if (Devices[1].sValue == "20"):  # Night Surveillance is on
                ...
                if idx in self.DTPerimetral:  # this is one of our DT
                    if "Status" in device:
                        ...
                        if device["Status"] == "On":
                            self.Perimetraltempo = datetime.now()
                            if Devices[2].nValue == 1:
                                Devices[8].Update(nValue = 3,sValue = "PERIMETRAL DETECTION : '{}'".format(device["Name"]))
                    else:
            ...
            if Devices[2].nValue == 1:
                if Devices[3].nValue == 1:
                    if Devices[6].nValue == 1:
                        ...
                        Devices[8].Update(nValue = 3,sValue = "New Perimetral Intrusion Detected")
                        self.Intrusion = True
                        self.Detectionchangedtime1 = datetime.now()
                ...
                if idx in self.DTNightAlarm:  # this is one of our presence/motion sensors
                    if "Status" in device:
                        ...
                        if device["Status"] == "On":
                            self.NightAlarmtempo = datetime.now()
                            if Devices[2].nValue == 1:
                                Devices[8].Update(nValue = 3,sValue = "GROUPE 1 DETECTION : '{}'".format(device["Name"]))
        ...
        if self.NightDetection:
            if Devices[5].nValue == 1:
                ...
                if self.Detectiondelay == 0:
                    Domoticz.Log("New intrusion...")
                    Devices[6].Update(nValue = 1,sValue = Devices[6].sValue)
                    Devices[8].Update(nValue = 3,sValue = "New Motion Detected in night zone")
                    self.Intrusion = True
                    self.Detectionchangedtime2 = datetime.now()
                else :
                ...
                if idx in self.DTNormalAlarm:  # this is one of our presence/motion sensors
                    if "Status" in device:
                        ...
                        if device["Status"] == "On":
                            self.NormalAlarmtempo = datetime.now()
                            if Devices[2].nValue == 1:
                                Devices[8].Update(nValue = 3,sValue = "GROUPE 2 DETECTION :  '{}'".format(device["Name"]))
        ...
        if self.NormalDetection:
            if Devices[6].nValue == 1:
                ...
                if self.Detectiondelay == 0:
                    Domoticz.Log("New intrusion...")
                    Devices[6].Update(nValue = 1,sValue = Devices[6].sValue)
                    Devices[8].Update(nValue = 3,sValue = "New Motion Detected in normal zone")
                    self.Intrusion = True
                    self.Detectionchangedtime3 = datetime.now()
                else :
                    if self.Detectiontempo + timedelta(seconds = self.Detectiondelay) <= now:
    ...
    def AlarmDetection(self):
        ...
        if self.Intrusion:
            if not self.Alarm:
                if self.Alarmondelay == 0:
                    ...
                    Devices[8].Update(nValue = 4,sValue = "Intrusion Detected - Alarm")
                else :
                    if self.Detectionchangedtime1 + timedelta(seconds = self.Alarmondelay) <= now:
                        Domoticz.Debug("Intrusion Detected and timer On period passed : Alarm ACTIVE !")
                        self.Alarm = True
                        Devices[7].Update(nValue = 1,sValue = Devices[7].sValue)
                        Devices[8].Update(nValue = 4,sValue = "Intrusion Detected - Alarm")
                    elif self.Detectionchangedtime2 + timedelta(seconds = self.Alarmondelay) <= now:
                        Domoticz.Debug("Intrusion Detected and timer On period passed : Alarm ACTIVE !")
                        self.Alarm = True
                        Devices[7].Update(nValue = 1,sValue = Devices[7].sValue)
                        Devices[8].Update(nValue = 4,sValue = "Intrusion Detected - Alarm")
                    elif self.Detectionchangedtime3 + timedelta(seconds = self.Alarmondelay) <= now:
                        Domoticz.Debug("Intrusion Detected and timer On period passed : Alarm ACTIVE !")
                        self.Alarm = True
                        Devices[7].Update(nValue = 1,sValue = Devices[7].sValue)
                        Devices[8].Update(nValue = 4,sValue = "Intrusion Detected - Alarm")
        ...
        if self.Alarm:
            if self.Detectionchangedtime1 + timedelta(seconds = (self.Alarmondelay + self.Alarmoffdelay)) <= now:
                # Reset of the zone detection for checking if new intrusion
                Domoticz.Debug("Alarm reset after timer OFF period !")
                self.Alarm = False
                Devices[7].Update(nValue = 0,sValue = Devices[7].sValue)
                Devices[8].Update(nValue = 3,sValue = "Alarm reset - Protection Armed")
                self.Intrusion = False
                Devices[6].Update(nValue = 0,sValue = Devices[6].sValue)
            elif self.Detectionchangedtime2 + timedelta(seconds = (self.Alarmondelay + self.Alarmoffdelay)) <= now:
                # Reset of the zone detection for checking if new intrusion
                Domoticz.Debug("Alarm reset after timer OFF period !")
                self.Alarm = False
                Devices[7].Update(nValue = 0,sValue = Devices[7].sValue)
                ...
                self.Intrusion = False
                Devices[6].Update(nValue = 0,sValue = Devices[6].sValue)
            elif self.Detectionchangedtime3 + timedelta(seconds = (self.Alarmondelay + self.Alarmoffdelay)) <= now:
                # Reset of the zone detection for checking if new intrusion
                Domoticz.Debug("Alarm reset after timer OFF period !")
                self.Alarm = False
                Devices[7].Update(nValue = 0,sValue = Devices[7].sValue)
