...
class BasePlugin:
    ...
    def verifAlarmState(self):
        ...
        now = datetime.now()
        #self.LevelControltime = datetime.now()
        LastActionAlarmLeval = self.LevelControltime
        Time = LastActionAlarmLeval.strftime("%H heure %M")
        #Domoticz.Debug("Last action on Alarm level is : {} and Idx {} is the last in the PS list !!! ".format(CalledSensorPositionInList,time))
        Domoticz.Debug("----> Last action on Alarm level is at : {} ".format(Time))
        ...
        if Devices[1].nValue == 0:
            ...
            if Devices[2].nValue == 1:
                ...
                if self.AlarmLevel == 1:
                    ...
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode périmétrique a {} mais elle est est en cours dinitialisation".format(Time))
                if self.AlarmLevel == 2:
                    Domoticz.Log("--Alarm - Check State : Initialization level Night...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode nuit a {} mais elle est est en cours dinitialisation".format(Time))
                if self.AlarmLevel == 3:
                    Domoticz.Log("--Alarm - Check State : Initialization level Total...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode général a {} mais elle est est en cours dinitialisation".format(Time))
