        ...
        if (Devices[9].nValue == 1):
            ...
            if (Devices[9].sValue == "10"):
                if self.PSControltime + timedelta(seconds = 5) <= now:
                    ...
                    if self.PreviousCalledSensorPositionInList <= PSnumbers :
                        #self.CalledPS = CheckParam("DT Checked", params[self.CalledSensorPositionInList], 1)
                        self.CalledPS = params[self.CalledSensorPositionInList]
                        Domoticz.Debug("PS Called is = {}".format(self.self.CalledPS))
                        self.PreviousCalledSensorPositionInList = (self.CalledSensorPositionInList + 1)
                        self.CalledSensorPositionInList = self.PreviousCalledSensorPositionInList
                        Domoticz.Debug("Next PS Called will be = {}".format(self.self.CalledPS))
