                ...
                if self.Armingtempo + timedelta(seconds = self.Armingondelay) <= now:
                    if Devices[2].nValue == 0:
                        ...
                        if self.Softrestartingtime + timedelta(minutes = 5) <= now:
                            ...
                            if self.Alexa:
                                os.system('sudo /home/pi/script/alexa_remote_control.sh -e speak:"L'initialisation est terminée. La surveillance a commencée. La protéction est active"')
        ...
        if self.Softrestartingtime + timedelta(seconds = 60) <= now:  # wait for any detection if domoticz is just restarting;
            ...
            if self.NightDetection:
                if Devices[6].nValue == 1:
                    ...
                    if self.Alexa:
                        os.system('sudo /home/pi/script/alexa_remote_control.sh -e speak:"Attention. Intrusion groupe 1 détéctée. Identification ou action requise "')
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
        ...
        if self.Softrestartingtime + timedelta(seconds = 60) <= now:  # wait for any detection if domoticz is just restarting;
            ...
            if self.NormalDetection:
                if Devices[6].nValue == 1:
                    ...
                    if self.Alexa:
                        os.system('sudo /home/pi/script/alexa_remote_control.sh -e speak:"Attention. Intrusion groupe 2 détéctée. Identification ou action requise "')
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
