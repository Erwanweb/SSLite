...
class BasePlugin:
    ...
    def __init__(self):
        ...
        self.DTtempoMS = datetime.now()
        self.PSnumbers = 0
        self.CalledPS = 0
        self.CalledDT = 0
        #self.CalledSensorPositionInList = 0
        #self.PreviousCalledSensorPositionInList = 0
        self.VerificationInFunction = False
        self.Telegram = False
        self.Alexa = False
        self.loglevel = None
    ...
    def onStart(self):
        ...
        cmd = 'sudo /home/pi/script/alexa_remote_control.sh -m Alarme Echo-Principal Echo-Cuisine'
        Domoticz.Debug("Starting Alexa API and activation of multiroom named Alarm")
        os.system(cmd)
        
        # reset of called PS or MS for verification
        #CalledSensorPositionInList = 0
        #PreviousCalledSensorPositionInList = 0
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if (Unit == 9):
            ...
            if (Devices[9].sValue == "10"):
                ...
                if self.Telegram:
                    TelegramAPI("Verification des PS - Capteurs perimetriques - Cela peut demander jusqua 2 minutes")
                if self.Alexa:
                    AlexaAPI("Vérification des capteurs périmétriques. cela peut demander jusqua 2 minutes")
                self.PSControltime = datetime.now()
            if (Devices[9].sValue == "20"):
                Devices[9].Update(nValue = 1,sValue = "20")
                Domoticz.Debug("Verification of MS - Motion Sensors")
                if self.Telegram:
                    TelegramAPI("Verification des MS - Capteurs de mouvements - Cela peut demander jusqua 2 minutes")
                if self.Alexa:
                    AlexaAPI("Vérification des capteurs de présence. cela peut demander jusqua 2 minutes")
                self.MSControltime = datetime.now()
    ...
    def onHeartbeat(self):
        ...
        if (Devices[9].nValue == 1):
            #self.AlarmReady = False
            if (Devices[9].sValue == "10"):
                if not self.VerificationInFunction:
                    CalledSensorPositionInList = 0
                    PreviousCalledSensorPositionInList = 0
                    Domoticz.Debug("Reset of sensor called and previous called position in list = '{}' and '{}'".format(CalledSensorPositionInList, PreviousCalledSensorPositionInList))
                if self.PSControltime + timedelta(seconds = 10) <= now:
                    noerror = True
                    #self.ControlSensortempo = datetime.now()
                    Domoticz.Log("Verification of PS Called...")
                    self.VerificationInFunction = True
                    params = parseCSV(Parameters["Mode1"])
                    Domoticz.Debug("PS Listed are = {}".format(params))
                    PSnumbers = len(params)
                    Domoticz.Debug("Quantity of PS Listed : {}".format(PSnumbers))
                    CalledSensorPositionInList = NextCalledSensorPositionInList
                    #Domoticz.Debug("we will call sensor position in list is = {}".format(CalledSensorPositionInList))
                    if CalledSensorPositionInList < PSnumbers :
                        ...
                        self.CalledPS = params[CalledSensorPositionInList]
                        Domoticz.Debug("PS Called is = {}".format(self.CalledPS))
                        PreviousCalledSensorPositionInList = (CalledSensorPositionInList + 1)
                        NextCalledSensorPositionInList = PreviousCalledSensorPositionInList
                        self.CalledPS = params[NextCalledSensorPositionInList]
                        Domoticz.Debug("Next PS Called will be = {}".format(self.CalledPS))
                        ...
                        if devicesAPI:
                            for device in devicesAPI["result"]:  # parse the perimetral sensors (PS) devices
                                idx = int(device["idx"])
                                if idx == self.CalledPS:  # this is one of our PS
                                    if "Status" in device:
                                        ...
                                        if device["Status"] == "On":
                                            ...
                                            if self.Alexa:
                                               AlexaAPI("{} Actif ".format(device["Name"]))
                    else:
                        self.PSControltime = datetime.now() + timedelta(seconds = 90)

                    if self.PSControltime + timedelta(seconds = 80) <= now: #At mini 60 seconds was past since asked verif PS or MS
                        Domoticz.Debug("Verif PS done...")
                        Devices[8].Update(nValue=3, sValue="Verif PS --- terminee")
                        self.VerificationInFunction = False
                        CalledSensorPositionInList = 0
                        PreviousCalledSensorPositionInList = 0
                        if self.DTtempoPS + timedelta(seconds = 120) >= now: #there is at mini 1 PS that was ON in the past 120 seconds
            ...
            if (Devices[9].sValue == "20"):
                if self.PSControltime + timedelta(seconds=5) <= now:
                    Devices[9].Update(nValue=0, sValue="0")
                    Domoticz.Log("Verification of MS...STOP")
                ...
                if Devices[2].nValue == 1:
                    if Devices[3].nValue == 1:
                        if Devices[6].nValue == 1:
                            ...
                            if self.Alexa:
                                AlexaAPI("Attention. Intrusion périmétrique détéctée. Identification requise")
                            self.Intrusion = True
                            self.Detectionchangedtime = datetime.now()
                    ...
                    if "Status" in device:
                        ...
                        if device["Status"] == "On":
                            ...
                            if Devices[2].nValue == 1:
                                ...
                                if self.Alexa:
                                    AlexaAPI("Détéction intrusion {}".format(device["Name"]))
        ...
        if self.Softrestartingtime + timedelta(seconds = 60) <= now:  # wait for any detection if domoticz is just restarting;
            ...
            if self.NightDetection:
                if Devices[6].nValue == 1:
                    ...
                    if self.Alexa:
                        AlexaAPI("Attention. Intrusion détéctée. groupe 1. Zone nuit. Identification requise")
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
        ...
        if self.Softrestartingtime + timedelta(seconds = 60) <= now:  # wait for any detection if domoticz is just restarting;
            ...
            if self.NormalDetection:
                if Devices[6].nValue == 1:
                    ...
                    if self.Alexa:
                        AlexaAPI("Attention. Intrusion détéctée. groupe 2. Zone générale. Identification requise")
                    self.Intrusion = True
                    self.Detectionchangedtime = datetime.now()
...
def AlexaAPI(APICall):
    ...
    cmd = 'sudo /home/pi/script/alexa_remote_control.sh -e speak:"{}"'.format(APICall)
