...
class BasePlugin:
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if self.LastCommand + timedelta(seconds=20) <= now: #tempo between 2 commands, also because alexa can send same 2 commands in very short time
                    ...
                    if not self.AlarmLevel == 0 : #alarm is on but in timer on period
                        ...
                        if (Devices[1].sValue == "10"):
                            ...
                            if (Devices[1].sValue == "20"):
                                ...
                                if self.Alexa:
                                    AlexaAPI("Protéction périmetrique activée. Initialisation en cours")
                                self.Armingtempo = datetime.now()
                                ...
                                if self.Alexa:
                                    AlexaAPI("Protéction mode nuit activée. Initialisation en cours")
                                self.Armingtempo = datetime.now()
                                ...
                                if self.Alexa:
                                    AlexaAPI("Protéction totale activée. Initialisation en cours")
                                self.Armingtempo = datetime.now()
                            ...
                            if self.Alexa:
                                AlexaAPI( "Commande de la protéction impossible. Vérifiez les capteurs périmétriques ou dites Alexa controle les capteurs actifs")
    ...
    def verifAlarmState(self):
        ...
        Day = LastActionAlarmLevel.today().strftime("%A") # in english
        DayoflastactiononAlarmLevel = LastActionAlarmLevel.today().strftime("%u")
        Today = datetime.now().strftime("%u")
        LastActionDayintext = Today
        if Today == DayoflastactiononAlarmLevel :
            ...
            if TDayoflastactiononAlarmLevel == "7":
                LastActionDayintext = "Dimanche"
        #Domoticz.Debug("Last action on Alarm level is : {} and Idx {} is the last in the PS list !!! ".format(CalledSensorPositionInList,time))
        Domoticz.Debug("----> Last action on Alarm level is on {} at {} ".format(Day, Time))
        Domoticz.Debug("----> In french : {} a {} ".format(LastActionDayintext, Time))


        if Devices[1].nValue == 0:
            Domoticz.Log("--Alarm - Check State : OFF...")
            if self.Alexa:
                AlexaAPI("La protection est désactivé depuis {} a {}".format(LastActionDayintext, Time))
        else:
            if Devices[2].nValue == 1:
                if self.AlarmLevel == 1:
                    Domoticz.Log("--Alarm - Check State : Active level Perimetral...")
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode périmétrique depuis {} a {}".format(LastActionDayintext, Time))
                if self.AlarmLevel == 2:
                    Domoticz.Log("--Alarm - Check State : Active level Night...")
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode nuit depuis {} a {}".format(LastActionDayintext, Time))
                if self.AlarmLevel == 3:
                    Domoticz.Log("--Alarm - Check State : Active level Total...")
                    if self.Alexa:
                        AlexaAPI("La protection est active en mode général depuis {} a {}".format(LastActionDayintext, Time))
            else:
                if self.AlarmLevel == 1:
                    Domoticz.Log("--Alarm - Check State : Initialization level Perimetral...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode périmétrique a {} mais elle est en cours dinitialisation".format(Time))
                if self.AlarmLevel == 2:
                    Domoticz.Log("--Alarm - Check State : Initialization level Night...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode nuit a {} mais elle est en cours dinitialisation".format(Time))
                if self.AlarmLevel == 3:
                    Domoticz.Log("--Alarm - Check State : Initialization level Total...")
                    if self.Alexa:
                        AlexaAPI("La protection a été activé en mode général a {} mais elle est en cours dinitialisation".format(Time))
