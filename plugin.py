...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if (Devices[9].nValue == 1):
            if (Devices[9].sValue == "10"):
                if self.PSControltime + timedelta(seconds = 20) <= now:
                        ...
                        for device in devicesAPI["result"]:  # parse the perimetral sensors (PS) devices
                            ...
                            if idx in self.DTPerimetral:  # this is one of our PS
                                if "Status" in device:
                                    ...
                                    Devices[8].Update(nValue=1, sValue="--- PS '{}' - '{}'".format(device["Name"], device["Status"]))
                                    if device["Status"] == "On":
                                        self.DTtempoPS = datetime.now()
                                        if self.Telegram:
                ...
                if self.ControlSensortempo + timedelta(seconds=60) <= now: #At mini 30 seconds was past since asked verif PS
                    Domoticz.Debug("Verif PS done...")
                    Devices[9].Update(nValue=0, sValue="0")
                    if self.DTtempoPS + timedelta(seconds=90) >= now: #there is not at mini 1 PS that was ON in the past 60 seconds
                        Domoticz.Debug("Verif PS -> SOME AR ON")
                        if self.Alexa:
                            AlexaAPI("Vérification des capteurs périmétriques terminé. Vérifiéz les capteurs actifs")
                    else:
