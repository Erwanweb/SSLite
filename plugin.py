        ...
        if (Devices[9].nValue == 1):
            ...
            if (Devices[9].sValue == "10"):
                if self.PSControltime + timedelta(seconds = 5) <= now:
                    ...
                    if self.PSControltime + timedelta(seconds = 30) <= now: #At mini 60 seconds was past since asked verif PS or MS
                        Domoticz.Debug("Verif PS done...")
                        Devices[8].Update(nValue=3, sValue="Verif PS --- terminee")
                        if self.DTtempoPS + timedelta(seconds = 60) >= now: #there is at mini 1 PS that was ON in the past 90 seconds
