...
class BasePlugin:
    ...
    def verifAlarmState(self):
        ...
        if self.Telegram:
            TelegramAPI(
                "----> Check days today is {} and last action is {} ".format(Today, DayoflastactiononAlarmLevel))
        ...
        if CalledSensorPositionInList < SensorsNumbers:
                ...
                for device in devicesAPI["result"]:  # parse the perimetral sensors (PS) devices
                    ...
                    if idx == self.CalledSensor:  # this is one of our PS
                        if "Status" in device:
                            ...
                            if device["Status"] == "On":
                                Devices[8].Update(nValue=3,
                                                  sValue="Verif MS en cours : --- '{}' - '{}'".format(device["Name"],
                                                                                                      device["Status"]))
                                self.DTtempoMS1 = datetime.now()
                                if self.Telegram:
                                    TelegramAPI("--- Actif : '{}'".format(device["Name"]))
            ...
            if (Devices[9].nValue == 1):  # self.VerifMS1 was called by Verif MS night action
                ...
                if self.DTtempoMS1 + timedelta(
                    ...
                    if self.Telegram:
                        TelegramAPI(
                            "Vérification des capteurs MS Groupe 1 - Night Zone terminé. Vérifiez les capteurs actifs")
                    if self.Alexa:
                        AlexaAPI(
                            "Vérification des capteurs de présence du Groupe 1 - Zone nuit terminé. Vérifiez les capteurs actifs")
                else:
        ...
        if CalledSensorPositionInList < SensorsNumbers:
            ...
            if (Devices[9].nValue == 1):  # self.VerifMS1 was called by Verif MS night action
                ...
                if self.DTtempoMS2 + timedelta(
                    ...
                    if self.Telegram:
                        TelegramAPI(
                            "Vérification des capteurs MS Groupe 2 - Zone Génerale terminé. Vérifiez les capteurs actifs")
                    if self.Alexa:
                        AlexaAPI(
                            "Vérification des capteurs de présence du Groupe 2 - Zone Génerale terminé. Vérifiez les capteurs actifs")
                else:
                    ...
                    if self.Alexa:
                        AlexaAPI(
                            "Vérification des capteurs de présence du Groupe 2 - Zone Génerale terminé. Toute est OK")
                Devices[9].Update(nValue=0, sValue="0")
            # Stopping check of sensor step by step
...
def TelegramAPI(APICall):
    resultJson = None
    url = "https://api.telegram.org/bot981531159:AAFPt2yI9BhfD2XdI3mUIgIdD2Jt_3A6kOw/sendMessage?chat_id={}&text={}".format(
