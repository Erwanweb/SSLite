    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if (Unit == 9):
            ...
            if (Devices[9].sValue == "10"):
                ...
                if self.Telegram:
                    TelegramAPI("Verification des PS - Capteurs perimetriques - Cela peut demander plusieurs minutes" )
                if self.Alexa:
                    AlexaAPI("Vérification des capteurs périmétriques. Cela peut demander plusieurs minutes - Je vous informerai des que la vérification sera terminé")
                self.PSControltime = datetime.now()
            if (Devices[9].sValue == "20"):
                ...
                if self.Telegram:
                    TelegramAPI("Verification des MS - Capteurs de mouvements - Cela peut demander plusieurs minutes")
                if self.Alexa:
                    AlexaAPI("Vérification des capteurs de présence. Cela peut demander plusieurs minutes - Je vous informerai des que la vérification sera terminé")
                self.MSControltime = datetime.now()
                                ...
                                if idx == self.CalledPS:  # this is one of our PS
                                    if "Status" in device:
                                        ...
                                        if device["Status"] == "On":
                                            ...
                                            if self.Alexa:
                                               AlexaAPI("Le capteur {} est actif ".format(device["Name"]))
                    else:
    ...
    def AlarmDetection(self):
        ...
        if self.Alarm:
            if self.Detectionchangedtime + timedelta(seconds = (self.Alarmondelay + self.Alarmoffdelay)) <= now:
                ...
                if self.Alexa:
                    AlexaAlarmAPI("Reset - Protection Armee")
                self.Intrusion = False
                Devices[6].Update(nValue = 0,sValue = Devices[6].sValue)
            else:
...
def AlexaAPI(APICall):
    # Check last alexa :
    #cmd = 'sudo /home/pi/script/alexa_remote_control.sh -lastalexa | sed "s/=/%/" | cut -d'%' -f1'
    #cmd = 'sudo /home/pi/script/alexa_remote_control.sh -lastalexa {} sed "s/=/%/" {} cut -d'%' -f1'.format("|")
    #cmd = 'sudo /home/pi/script/alexa_remote_control.sh -lastalexa {} sed "s/=/%/"'.format("|")
    #os.system(cmd)
    #Id = response.read()
    #Domoticz.Debug("Calling Alexa API for knowing last alexa speaking : {}".format(Id))
    #os.system(cmd)
    cmd = 'sudo /home/pi/script/alexa_remote_control.sh -lastalexa {} cut -d"=" -f1'.format("|")
    output = sp.getoutput(cmd)
    Domoticz.Debug("Calling Alexa API for knowing last alexa speaking : {}".format(output))
