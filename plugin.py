...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if (Devices[1].nValue <= 1):  # Surveillance is off but we check if perimetral is ok, if not, unable to turn on protection
            if Devices[3].nValue == 0:
                Devices[1].Update(nValue = 0,sValue = "10")
            else :
                Devices[1].Update(nValue = 0,sValue = "0")

        if (Devices[1].nValue <= 1):  # Surveillance is off
