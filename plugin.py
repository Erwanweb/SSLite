        ...
        if (Devices[9].nValue == 1):
            ...
            if (Devices[9].sValue == "10"):
                if self.PSControltime + timedelta(seconds = 5) <= now:
                        ...
                        for device in devicesAPI["result"]:  # parse the perimetral sensors (PS) devices
                            ...
                            if idx in self.DTPerimetral:  # this is one of our PS
                                if "Status" in device:
                                    ...
                                    if device["Status"] == "On":
                                        Devices[8].Update(nValue=3, sValue="Verif PS : --- '{}' - '{}'".format(device["Name"], device["Status"]))
                                        self.DTtempoPS = datetime.now()
                                        if self.Telegram:
                    ...
                    if self.PSControltime + timedelta(seconds = 40) <= now: #At mini 60 seconds was past since asked verif PS or MS
                        Domoticz.Debug("Verif PS done...")
                        if self.DTtempoPS + timedelta(seconds = 70) >= now: #there is at mini 1 PS that was ON in the past 90 seconds
                            Domoticz.Debug("Verif PS -> SOME AR ON")
                            if self.Alexa:
                                AlexaAPI("Vérification des capteurs périmétriques terminé. Vérifié les capteurs actifs")
                        else:
